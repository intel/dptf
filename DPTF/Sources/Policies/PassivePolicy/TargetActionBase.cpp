...
TargetActionBase::TargetActionBase(
	...
	, m_participantTracker(participantTracker)
	, m_trt(trt)
	, m_callbackScheduler(callbackScheduler)
...
std::vector<UIntN> TargetActionBase::filterDomainList(
	const std::vector<UIntN>& domainList,
	const std::vector<UIntN>& filterList) const
{
	...
	const tuple<UIntN, DomainPriority, UtilizationStatus>& right)
{
	try
	{
		...
		{
			if (get<2>(left).getCurrentUtilization() > get<2>(right).getCurrentUtilization())
			{
				return true;
			}
			else
...
std::string TargetActionBase::constructMessageForSources(
	...
	UIntN target,
	const std::vector<UIntN>& sources)
{
