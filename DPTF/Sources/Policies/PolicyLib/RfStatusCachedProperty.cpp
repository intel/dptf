...
RfProfileDataSet RfStatusCachedProperty::getRadioProfile(void)
{
...
void RfStatusCachedProperty::refreshData(void)
{
	...
	{
		m_profileData =
			getPolicyServices().domainRfProfileStatus->getRfProfileDataSet(getParticipantIndex(), getDomainIndex());
	}
}
