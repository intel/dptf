...
void PolicyCallbackScheduler::suspend(ParticipantRole::Type participantRole, UIntN participantIndex, const TimeSpan& time)
{
...
Bool PolicyCallbackScheduler::hasCallbackWithinTimeRange(UIntN target, const TimeSpan& beginTime, const TimeSpan& endTime) const
{
...
std::shared_ptr<XmlNode> PolicyCallbackScheduler::getStatusForParticipant(ParticipantRole::Type participantRole, UIntN participantIndex)
{
    ...
    try
    {
        ...
        status->addChild(
            XmlNode::createDataElement("participant_role", ParticipantRole::ToString(participantRole)));
        ...
        if (scheduledCallback != m_schedule.end())
        {
            auto expireTime = scheduledCallback->second.getTimeStamp() +
                scheduledCallback->second.getTimeDelta();
            auto currentTime = getCurrentTime();
            ...
            if (expireTime >= currentTime)
            {
                ...
                status->addChild(
                    XmlNode::createDataElement("current_polling_period", currentPoll));
            }
            else
            {
                status->addChild(XmlNode::createDataElement("time_until_expires", Constants::InvalidString));
                status->addChild(
                    XmlNode::createDataElement("current_polling_period", Constants::InvalidString));
            }
        }
        else
        {
...
void PolicyCallbackScheduler::scheduleCallback(UIntN participantIndex, const TimeSpan& currentTime, const TimeSpan& pollTime)
{
