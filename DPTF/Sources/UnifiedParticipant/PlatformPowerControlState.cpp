...
void PlatformPowerControlState::captureLimit(PlatformPowerLimitType::Type limitType, Power& limit, Bool& limitValid)
{
	try
	{
		limit =
			m_control->getPlatformPowerLimit(m_control->getParticipantIndex(), m_control->getDomainIndex(), limitType);
		limitValid = true;
	}
	catch (...)
	{
