...
WorkItemMatchCriteria::WorkItemMatchCriteria(void)
	: m_matchCriteriaAdded(false)
	, m_testFrameworkEventType(false)
	, m_frameworkEventType(FrameworkEvent::Max)
	, m_testUniqueId(false)
	, m_uniqueId(Constants::Invalid)
	, m_testParticipantIndex(false)
	, m_participantIndex(Constants::Invalid)
	, m_testDomainIndex(false)
	, m_domainIndex(Constants::Invalid)
	, m_testPolicyIndex(false)
...
Bool WorkItemMatchCriteria::testAgainstMatchList(
	...
	UIntN policyIndex) const
{
	...
	if ((m_testDomainIndex == true) && ((domainIndex == Constants::Invalid) || (m_domainIndex != domainIndex)))
	{
		return false;
	}

	if ((m_testPolicyIndex == true) && ((policyIndex == Constants::Invalid) || (m_policyIndex != policyIndex)))
	{
		return false;
	}
