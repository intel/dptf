...
TemperatureThresholdArbitrator::TemperatureThresholdArbitrator(DptfManagerInterface* dptfManager)
	...
	, m_arbitratedTemperatureThresholds(
	...
	const Temperature hysteresis)
{
	...
	ManagerMessage message =
		ManagerMessage(m_dptfManager, FLF, "Temperature arbitration data is being updated for a policy request.");
	message.setPolicyIndex(policyIndex);
	message.addMessage("Current Temperature", currentTemperature);
	message.addMessage(
		"Requested Aux0/Aux1",
		temperatureThresholds.getAux0().toString() + "/" + temperatureThresholds.getAux1().toString());
...
void TemperatureThresholdArbitrator::updateTemperatureDataForPolicy(
	UIntN policyIndex,
	const TemperatureThresholds& temperatureThresholds)
{
...
Bool TemperatureThresholdArbitrator::findNewTemperatureThresholds(const Temperature& currentTemperature)
{
	...
	for (auto thresholds = m_requestedTemperatureThresholds.begin();
		 thresholds != m_requestedTemperatureThresholds.end();
		 thresholds++)
	{
...
void TemperatureThresholdArbitrator::addArbitrationDataToMessage(ManagerMessage& message, const std::string& title)
{
	...
	message.addMessage(
		"Arbitrated Aux0/Aux1",
		m_arbitratedTemperatureThresholds.getAux0().toString() + "/"
	...
	{
		message.addMessage(
			"Policy " + std::to_string(thresholds->first),
			thresholds->second.getAux0().toString() + "/" + thresholds->second.getAux1().toString());
	}
}