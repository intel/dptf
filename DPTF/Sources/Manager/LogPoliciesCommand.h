...
class dptf_export LogPoliciesCommand : public CommandHandler
{
public:
	LogPoliciesCommand(DptfManagerInterface* dptfManager, const std::shared_ptr<IFileIo>& fileIo);
	~LogPoliciesCommand() override;
	std::string getCommandName() const override;
	void execute(const CommandArguments& arguments) override;

private:
	std::shared_ptr<CommandDispatcher> m_logPoliciesCommandDispatcher;
