...
class TableObjectCommand : public CommandHandler
{
public:
	TableObjectCommand(DptfManagerInterface* dptfManager);
	virtual ~TableObjectCommand();
	virtual std::string getCommandName() const override;
	virtual void execute(const CommandArguments& arguments) override;

private:
	std::shared_ptr<CommandDispatcher> m_tableobjectCommandDispatcher;
