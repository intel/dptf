...
class dptf_export ConfigReloadCommand : public CommandHandler
{
public:
	ConfigReloadCommand(DptfManagerInterface* dptfManager);
	virtual ~ConfigReloadCommand();
	virtual std::string getCommandName() const override;
	virtual void execute(const CommandArguments& arguments) override;

private:

	void reloadConfigurations() const;
	void throwIfBadArguments(const CommandArguments& arguments);
	void throwIfInvalidConfigurationManager(
