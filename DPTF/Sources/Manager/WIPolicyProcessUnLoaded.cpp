...
WIPolicyProcessUnLoaded::WIPolicyProcessUnLoaded(
...
WIPolicyProcessUnLoaded::~WIPolicyProcessUnLoaded(void)
{
}

void WIPolicyProcessUnLoaded::onExecute(void)
{
	...
	for (const auto& policyIndex : policyIndexes)
	{
		try
		{
			...
			policy->executePolicyProcessUnLoaded(m_processId);
		}
		...
		{
			writeWorkItemErrorMessagePolicy(ex, "Policy::executePolicyProcessUnLoaded", policyIndex);
		}
	}
}