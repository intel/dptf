...
#ifdef ESIF_LZMA_SDL_ENHANCEMENTS
...
void LzmaEnc_SaveState(CLzmaEncHandle pp)
{
  ...
  {
    MyMemcpy(dest->isMatch[i], p->isMatch[i], sizeof(p->isMatch[i]));
	MyMemcpy(dest->isRep0Long[i], p->isRep0Long[i], sizeof(p->isRep0Long[i]));
  }
  for (i = 0; i < kNumLenToPosStates; i++)
	  MyMemcpy(dest->posSlotEncoder[i], p->posSlotEncoder[i], sizeof(p->posSlotEncoder[i]));
  MyMemcpy(dest->isRep, p->isRep, sizeof(p->isRep));
  MyMemcpy(dest->isRepG0, p->isRepG0, sizeof(p->isRepG0));
  MyMemcpy(dest->isRepG1, p->isRepG1, sizeof(p->isRepG1));
  MyMemcpy(dest->isRepG2, p->isRepG2, sizeof(p->isRepG2));
  MyMemcpy(dest->posEncoders, p->posEncoders, sizeof(p->posEncoders));
  MyMemcpy(dest->posAlignEncoder, p->posAlignEncoder, sizeof(p->posAlignEncoder));
  MyMemcpy(dest->reps, p->reps, sizeof(p->reps));
  MyMemcpy(dest->litProbs, p->litProbs, ((UInt32)0x300 << p->lclp) * sizeof(CLzmaProb));
}
...
void LzmaEnc_RestoreState(CLzmaEncHandle pp)
{
  ...
  MyMemcpy(dest->posAlignEncoder, p->posAlignEncoder, sizeof(p->posAlignEncoder));
  MyMemcpy(dest->reps, p->reps, sizeof(p->reps));
  MyMemcpy(dest->litProbs, p->litProbs, ((UInt32)0x300 << dest->lclp) * sizeof(CLzmaProb));
}
...
{
  return p->repLenEnc.prices[posState][SDL_SYMBOLIDX((size_t)len - LZMA_MATCH_LEN_MIN)] +
