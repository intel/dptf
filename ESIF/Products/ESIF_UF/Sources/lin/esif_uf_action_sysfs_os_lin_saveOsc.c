...
static eEsifError HandleOscRequest(const struct esif_data_complex_osc *oscPtr, const char *cur_node_name)
{
	...
	if (SysfsSetString(g_ManagerSysfsPath, SYSFS_CURRENT_UUID, dptfGuid) < 0) {
	        ESIF_TRACE_DEBUG("Legacy OSC interface\n");
	        printf("Legacy OSC interface\n");
                newOsc = false;
	}

	isDptfGuid = esif_ccb_strncmp(guidStr, dptfGuid, ESIF_GUID_PRINT_SIZE);
	if (newOsc) {
		/* New OSC interface */
		if (!isDptfGuid) {
			/* Handle only Dptf GUID */
			...
			oscCap = oscPtr->capabilities;
		} else {
			/* Ignore policy GUID */
			osCap = -1;
		}
	} else {
		/* Legacy OSC interface */
		if (SysfsSetString(g_ManagerSysfsPath, SYSFS_CURRENT_UUID, guidStr) < 0) {
			ESIF_TRACE_WARN("Failed to set _OSC for GUID: %s\n", guidStr);
			rc = ESIF_E_PRIMITIVE_ACTION_FAILURE;
			goto exit;
