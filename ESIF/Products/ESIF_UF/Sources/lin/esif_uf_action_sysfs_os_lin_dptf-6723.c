...
printf("TTTT: guidStr: %s\n", guidStr);
printf("TTTT: oscPtr->capabilities: 0x%x\n", oscPtr->capabilities);

	if (oscPtr->capabilities) {	// DPTF to take over thermal control
printf("TTTT: setting %s/%s to enabled\n", cur_node_name, "mode");
		if (SysfsSetString(cur_node_name, "mode", "enabled") < 0) {
			rc = ESIF_E_PRIMITIVE_ACTION_FAILURE;
			goto exit;
		}
	} else {
printf("TTTT: setting %s/%s to disabled\n", cur_node_name, "mode");
...
#ifdef NAY
	/* TTTT */
	if (0 == esif_ccb_strncmp(guidStr, "B23BA85D-C8B7-3542-88DE-8DE2FFCFD698", ESIF_GUID_PRINT_SIZE)) {
		printf("TTTT: received the special GUID\n");
		static u32 prevOscCap;

		if ((oscPtr->capabilities != prevOscCap) || !oscPtr->capabilities) {
			if (SysfsSetString(g_ManagerSysfsPath, SYSFS_CURRENT_UUID, "reset") < 0) {
				ESIF_TRACE_WARN("Failed to set _OSC for GUID: %s\n", "reset");
				rc = ESIF_E_PRIMITIVE_ACTION_FAILURE;
				goto exit;
			}
		} else if (!(oscPtr->capabilities & 0x1)) {
			ESIF_TRACE_WARN("Dynamic Tuning is not enabled\n");
			printf("Dynamic Tuning is not enabled\n");
			rc = ESIF_E_PRIMITIVE_ACTION_FAILURE;
			goto exit;
		}
		...
		for (int i = 1; i <= 3; i++) {
			if (oscPtr->capabilities & (0x1 << i)) {
				printf("TTTT: oscPtr->capabilities & (0x1 << i):0x%x\n", oscPtr->capabilities & (0x1 << i));
				esif_ccb_strncpy(guidStr, policies_guid[i - 1], ESIF_GUID_PRINT_SIZE);
				printf("TTTT: %d bit is set; guidStr:%s\n", i, guidStr);
...
#endif
printf("TTTT: [else] setting %s to %s/%s\n", guidStr, g_ManagerSysfsPath, SYSFS_CURRENT_UUID);
	if (SysfsSetString(g_ManagerSysfsPath, SYSFS_CURRENT_UUID, guidStr) < 0) {
		ESIF_TRACE_WARN("Failed to set _OSC for GUID: %s\n", guidStr);
		rc = ESIF_E_PRIMITIVE_ACTION_FAILURE;
		goto exit;
